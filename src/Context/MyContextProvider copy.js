...
export const MyContextProvider = ({ children }) => {
  ...
  const sendTickets = (data, type, showLoader = true) => {
    ...
    const sendSMSAndAlert = async () => {
      try {
        await HandleSendSMS(number, encodedMessage, organizerApiKey, organizerSenderId, config_status, name, qty, ticketName, eventName);
        const values = {
          name,
          number,
          eventName: eventName,
